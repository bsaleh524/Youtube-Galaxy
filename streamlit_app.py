...
# --- Configuration ---
PARQUET_URL = "https://github.com/bsaleh524/Youtube-Galaxy-Streamlit-App/releases/download/v1.0/starmap_data.parquet"
...
def render_starmap(df):
    ...
    with col_info:
        if selected_cluster != "All":
            ...
            st.caption(f"{len(cluster_df)} creators in this group")
...
def main():
    ...
    """)
    st.link_button("🔔 Subscribe to SpookyPharaoh", "https://www.youtube.com/@SpookyPharaoh?sub_confirmation=1")

    tabs = st.tabs(["Galaxy", "Controversy Predictor"])

    with tabs[0]:
        try:
            df_map = load_starmap_data(PARQUET_URL)
        except Exception as e:
            st.error(f"Failed to load star map data: {e}")
            df_map = None
        if df_map is not None:
            st.metric("Total Creators in the Galaxy", f"{len(df_map):,}")
            if 'z' not in df_map.columns:
                ...
                st.markdown("---")
                st.subheader("📊 Creator Distribution by Group")
                cluster_col = 'cluster_name' if 'cluster_name' in df_map.columns else 'cluster_id'
                all_clusters = df_map[cluster_col].value_counts().reset_index()
                all_clusters.columns = ['Group', 'Count']
                fig_dist = px.bar(all_clusters, x='Count', y='Group', orientation='h',
                                  title="Creator Groups",
                                  color='Count', color_continuous_scale='Viridis')
                fig_dist.update_layout(yaxis={'categoryorder': 'total ascending'},
                                       height=max(600, len(all_clusters) * 22),
